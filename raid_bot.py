...
SPECIAL_ROLES = ['Kite', 'Cannons', 'Reflect', 'Tower', 'Back Warg', 'Hand Kite', 'Super Speed', 
...
async def on_ready():
    ...
    check_events.start()  # Start checking for reminders

@bot.tree.command(name="create_event", description="Create a new event with core and special role limits")
async def create_event(
    ...
    g3_slots: int = 0,
    lamps_slots: int = 0,
    kite_push_slots: int = 0,
    off_tank_slots: int = 0,
    portals_slots: int = 0,
    pylons_slots: int = 0
):
    """
    Create an event with core and special role limits
    ...
    Core roles are required, special roles are optional
    """
    try:
        # Validate event type
        valid_event_types = ["Fractals", "Raid", "WvW", "Meta", "Guild Missions", "PvP", "Chill Sessions"]
        if event_type not in valid_event_types:
            await interaction.response.send_message(f"Invalid event type! Choose from: {', '.join(valid_event_types)}", ephemeral=True)
            return
        ...
        core_slots = (fill_slots + heal_slots + qheal_slots + aheal_slots + powerdps_slots + 
                     condidps_slots + qdps_slots + adps_slots + tank_slots)
        ...
        events[event_id] = {
            ...
            'special_role_limits': {
                ...
                'G3': g3_slots,
                'Lamps': lamps_slots,
                'Kite/Push': kite_push_slots,
                'Off Tank': off_tank_slots,
                'Portals': portals_slots,
                'Pylons': pylons_slots
            },
        ...
        await interaction.response.send_message("Wrong time format! Use: YYYY-MM-DD HH:MM (like 2024-12-25 20:00)", ephemeral=True)
...
def create_event_embed(event_id):
    ...
    # Role emojis
    role_emojis = {
        'Fill': '👥', 'Heal': '❤️', 'Qheal': '💚', 'Aheal': '💙',
        'PowerDPS': '⚔️', 'CondiDPS': '🗡️', 'QDPS': '🏹', 'ADPS': '🔥',
        'Tank': '🛡️', 'Kite': '🏃', 'Cannons': '💣', 'Reflect': '🔄',
        'Tower': '🗼', 'Back Warg': '🐺', 'Hand Kite': '✋', 'Super Speed': '⚡',
        'Throw': '🎯', 'G1': '1️⃣', 'G2 Backups': '2️⃣', 'G3': '3️⃣',
        'Lamps': '💡', 'Kite/Push': '🔄', 'Off Tank': '🛡️', 'Portals': '🌀', 'Pylons': '⚡'
    }
    ...
    for role, limit in event['core_role_limits'].items():
        if limit > 0:
            ...
            current = sum(1 for p in event['participants'].values() if p['core_role'] == role)
            
            # Get participant names
            participant_names = [p['name'] for p in event['participants'].values() if p['core_role'] == role]
    ...
    special_roles_exist = any(limit > 0 for limit in event['special_role_limits'].values())
    if special_roles_exist:
        ...
        for role, limit in event['special_role_limits'].items():
            if limit > 0:
                ...
                current = sum(1 for p in event['participants'].values() if p.get('special_role') == role)
                
                # Get participant names
                participant_names = [p['name'] for p in event['participants'].values() if p.get('special_role') == role]
                if participant_names:
                    names_text = "\n".join(participant_names)
                else:
                    names_text = "Empty"
...
class CoreRoleSelect(discord.ui.Select):
    def __init__(self, event_id):
        ...
        role_emojis = {
            'Fill': '👥', 'Heal': '❤️', 'Qheal': '💚', 'Aheal': '💙',
            'PowerDPS': '⚔️', 'CondiDPS': '🗡️', 'QDPS': '🏹', 'ADPS': '🔥', 'Tank': '🛡️'
        }
        ...
        super().__init__(placeholder="⭐ Choose your CORE role (required)...", options=options)
    ...
    async def handle_role_selection(self, interaction, role_type):
        ...
        if role_type == 'core':
            ...
            current_core = sum(1 for p in event['participants'].values() if p['core_role'] == selected_role)
            if current_core >= event['core_role_limits'][selected_role]:
                await interaction.response.send_message(f"{selected_role} is full!", ephemeral=True)
                return
...
class SpecialRoleSelect(discord.ui.Select):
    def __init__(self, event_id):
        ...
        options = [discord.SelectOption(label="None (Remove special role)", value="None", emoji="❌")]
        
        role_emojis = {
            'Kite': '🏃', 'Cannons': '💣', 'Reflect': '🔄', 'Tower': '🗼',
            'Back Warg': '🐺', 'Hand Kite': '✋', 'Super Speed': '⚡', 'Throw': '🎯',
            'G1': '1️⃣', 'G2 Backups': '2️⃣', 'G3': '3️⃣', 'Lamps': '💡',
            'Kite/Push': '🔄', 'Off Tank': '🛡️', 'Portals': '🌀', 'Pylons': '⚡'
        }
        ...
        super().__init__(placeholder="🎯 Choose special role (optional)...", options=options)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if user_id not in event['participants']:
            await interaction.response.send_message("You must pick a core role first!", ephemeral=True)
            return
        ...
        if selected_role == "None":
            ...
            current_special = sum(1 for p in event['participants'].values() if p.get('special_role') == selected_role)
...
class EventView(discord.ui.View):
    ...
    async def leave_event(self, interaction: discord.Interaction, button: discord.ui.Button):
        event_id = self.children[0].event_id  # Get event_id from first select menu
...
async def check_events():
    ...
    for event_id, event in events.items():
        ...
        # 1 hour reminder
        if not event['reminded_1h'] and now >= event_time - timedelta(hours=1):
            await send_reminder(event, "1 hour")
            event['reminded_1h'] = True
        
        # 30 minute reminder
        if not event['reminded_30m'] and now >= event_time - timedelta(minutes=30):
...
# Run the bot
bot.run('d40e3628d470f8d0fa8f049f786b56147cde68db12730cafffc33e7b2602d65d')